...
DeliveryLoc = {
    {x = -20.8601, y = 239.5079, z = 109.55},
    {x = 346.1981, y = 440.3298, z = 148.04},
    {x = 1060.567, y = -378.151, z = 68.231},
    {x = -419.380, y = 221.2204, z = 83.395},
    {x = -28.9154, y = -234.038, z = 46.293},
    {x = -564.509, y = 94.43460, z = 61.371},
    {x = -634.671, y = 209.2258, z = 74.261},
    {x = -1215.89, y = 457.8915, z = 92.063},
    {x = -446.090, y = 686.3715, z = 153.11},
    {x = -951.236, y = 682.5476, z = 153.57},
    {x = 101.7218, y = -819.345, z = 31.316},
    {x = 1264.716, y = -702.845, z = 64.906},
    {x = 996.8353, y = -729.523, z = 57.815},
    {x = -768.979, y = -355.954, z = 37.333},
    {x = -1721.97, y = -482.736, z = 41.619},
    {x = -1931.24, y = 362.4902, z = 93.972},
    {x = -1208.59, y = -1384.21, z = 4.0769},
    {x = -578.425, y = -1070.13, z = 22.329},
    {x = 53.04883, y = -1478.86, z = 29.285},
    {x = 1365.535, y = -1721.73, z = 65.634},
    {x = 440.5135, y = -1829.72, z = 28.361},
}
...
Citizen.CreateThread(function()
    ...
    while true do
        Citizen.Wait(7)
        if primeItem ~= nil then
            local ped = PlayerPedId()
            local coords = GetEntityCoords(ped)
            for i = 1, #DeliveryLoc do
                dst = GetDistanceBetweenCoords(coords, DeliveryLoc[i].x, DeliveryLoc[i].y, DeliveryLoc[i].z)
                if dst <= 2.5 and primeSize == nil and primeItem ~= nil then
                    QBCore.Functions.DrawText3D(DeliveryLoc[i].x, DeliveryLoc[i].y, DeliveryLoc[i].z, "[E] Kargoyu bırak")
                    if IsControlJustPressed(0, 38) then
                        cash = math.floor(math.random(30, 80))
                        if primeItem == "koli2" then
                            cash = math.floor(math.random(110,180))
                             end
                        if primeItem == "koli3" then
                            cash = math.floor(math.random(250, 350))
                        end
                        print(cash)
                        DeleteObject(koliObj)
                        QBCore.Functions.Notify("Kargoyu bıraktın.", "success")
                        TriggerServerEvent('QBCore:Server:RemoveItem', primeItem, 1)
                        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[primeItem], "remove")
                        TriggerServerEvent('QBCore:Server:AddItem', "cash", cash)                
                        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["cash"], "add")
                        primeItem = nil
                        RemoveBlip(DeliveryBlip2)
                        Citizen.Wait(300000)
                        InShipping = nil
                    end
                end
...
RegisterNUICallback('PrimeOrder', function(data,cb)
    if not InShipping then
        cb(true)
        primeSize = data.label
        primeItem = data.item
        InShipping = true
        SetPrimeRoute()
    else
...
propArray = {
    ["Küçük Koli"] = "prop_vlastprime_25",
    ["Orta Boy Koli"] = "prop_vlastprime_50",
    ["Büyük Boy Koli"] = "prop_vlastprime_80"
}

RegisterNetEvent("vprime:createObject")
AddEventHandler("vprime:createObject", function(array)
    local Player = PlayerPedId()
    local x,y,z = table.unpack(GetEntityCoords(Player))
    objElinde = true
    koliObj = CreateObject(GetHashKey(""), x,y,z +0.2, true,true,true)
    TaskPlayAnim(PlayerPedId(), "anim@heists@box_carry@", "idle", 2.0, 2.0, -1, 51, 0, false, false, false)
    AttachEntityToEntity(koliObj, Player, GetPedBoneIndex(Player, 60309), 0.025, 0.08, 0.255, -145.0, 290.0, 0.0, true, true, false, true, 1, true)
end)

RegisterNetEvent('vprime-getOrder')
AddEventHandler('vprime-getOrder', function()
    RemoveBlip(DeliveryBlip)
    if primeSize ~= nil then
        QBCore.Functions.Notify(""..primeSize.." boyutundaki koliyi aldın, konuma doğru ilerle.", "inform")
        TriggerEvent("vprime:createObject", primeSize)
        TriggerServerEvent('QBCore:Server:AddItem', primeItem, 1)
        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[primeItem], "add")
        if primeItem == "koli1" then 
            va = math.random(1, #DeliveryLocations)
        elseif primeItem == "koli2" then
            va = math.random(1, #DeliveryLocationsMid)
        elseif primeItem == "koli3" then
            va = math.random(1, #DeliveryLocationsHigh)
        end
        primeSize = nil
        SetDeliveryRoute(va)
    else
        QBCore.Functions.Notify("Alabileceğin bir kargo yok.")
    end
end)

RegisterNUICallback('PrimeList', function(data,cb)
    cb(Config.VPrime)
end)


function SetPrimeRoute()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    ...
    DeliveryBlip = AddBlipForCoord(132.8546, 96.32911, 83.507)
    ...
    AddTextComponentSubstringPlayerName("Prime Depo")
    EndTextCommandSetBlipName(DeliveryBlip)
    SetBlipRoute(DeliveryBlip, true)
end


function SetDeliveryRoute(t)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)

    if DeliveryBlip2 ~= nil then
        RemoveBlip(DeliveryBlip2)
    end
    if primeItem == "koli1" then DeliveryBlip2 = AddBlipForCoord(DeliveryLocations[t].x, DeliveryLocations[t].y, DeliveryLocations[t].z) end
    if primeItem == "koli2" then DeliveryBlip2 = AddBlipForCoord(DeliveryLocationsMid[t].x, DeliveryLocationsMid[t].y, DeliveryLocationsMid[t].z) end
    if primeItem == "koli3" then DeliveryBlip2 = AddBlipForCoord(DeliveryLocationsHigh[t].x, DeliveryLocationsHigh[t].y, DeliveryLocationsHigh[t].z) end
    SetBlipSprite(DeliveryBlip2, 1)
    SetBlipDisplay(DeliveryBlip2, 2)
    SetBlipScale(DeliveryBlip2, 1.0)
    SetBlipAsShortRange(DeliveryBlip2, false)
    SetBlipColour(DeliveryBlip2, 5)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentSubstringPlayerName("Kargo Alıcısı")
    EndTextCommandSetBlipName(DeliveryBlip2)
    SetBlipRoute(DeliveryBlip2, true)
end
...
-- RegisterNUICallback("aiakos-inputcheck", function(data)
