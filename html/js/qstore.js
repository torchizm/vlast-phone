...
SetupAppstore = function(data) {
    ...
    $.each(data.StoreApps, function(i, app){
        if (data.PhoneData.InstalledApps[i] == null || data.PhoneData.InstalledApps[i] == undefined) {
            ...
            var elem = '<div class="storeapp" id="app-'+i+'" data-app="'+i+'"><div style="background-image: url(./img/apps/'+app.image+'.png)" class="storeapp-icon"></div><div class="storeapp-title">'+app.title+'</div> <div class="storeapp-creator">'+app.creator+'</div><div class="storeapp-download"><i class="fas fa-download"></i></div></div>'
            ...
            var elem = '<div class="storeapp" id="app-'+i+'" data-app="'+i+'"><div style="background-image: url(./img/apps/'+app.image+'.png)" class="storeapp-icon"></div><div class="storeapp-title">'+app.title+'<span style="font-size: 1vh;"> - Installed</span></div> <div class="storeapp-creator">'+app.creator+'</div><div class="storeapp-remove"><i class="fas fa-trash"></i></div></div>'
            $(".store-apps").append(elem);
            app.app = i;
            $("#app-"+i).data('AppData', app);
        }
        ...
        $(".download-progressbar-fill").animate({
            ...
            $.post('http://qb-phone/InstallApplication', JSON.stringify({
                ...
                if (Installed) {
                    ...
                    var blockedapp = IsAppJobBlocked(Installed.data.blockedjobs, NM.Phone.Data.PlayerJob.name)
                    if ((!Installed.data.job || Installed.data.job === NM.Phone.Data.PlayerJob.name) && !blockedapp) {
                    ...
                    NM.Phone.Data.Applications[Installed.data.app] = Installed.data;
...
$(document).on('click', '.storeapp-remove', function(e){
    ...
    NM.Phone.Data.Applications[AppData.app] = null;
...
/* 07.12.2021 11:50 pushed to github.com/torchizm/vlast-phone */